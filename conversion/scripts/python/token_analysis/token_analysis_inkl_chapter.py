...
OUTPUT_FILE_NAME = "token_stats_mistralai_Mixtral-8x22B-Instruct-v0.1.tsv"
...
def main():
    ...
    # ==========================================
    # 2. KAPITEL STATISTIK (CHUNKS)
    # ==========================================

    # Hier nehmen wir ALLE Kapitel aus allen Büchern zusammen
    narrative_chunks = df[df['type'] == 'narrative']

    print("\n" + "="*85)
    print(f" {'STATISTIK: KAPITEL-EBENE (Chunks)':^80} ")
    print("="*85)
    print(f"Anzahl untersuchter Kapitel/Chunks gesamt: {len(narrative_chunks)}")

    if not narrative_chunks.empty:
        # Berechnung der Statistiken über alle Chunks hinweg
        chunk_stats = narrative_chunks[['words_plain', 'tokens_plain', 'tokens_markup']].agg(['mean', 'median', 'min', 'max'])

        print(f"{'Metrik':<15} | {'Wörter':>10} | {'Token(Text)':>12} | {'Token(HTML)':>12}")
        print("-" * 56)
        print(f"{'Durchschnitt':<15} | {chunk_stats.loc['mean', 'words_plain']:10,.0f} | {chunk_stats.loc['mean', 'tokens_plain']:12,.0f} | {chunk_stats.loc['mean', 'tokens_markup']:12,.0f}")
        print(f"{'Median':<15} | {chunk_stats.loc['median', 'words_plain']:10,.0f} | {chunk_stats.loc['median', 'tokens_plain']:12,.0f} | {chunk_stats.loc['median', 'tokens_markup']:12,.0f}")
        print(f"{'Maximum':<15} | {chunk_stats.loc['max', 'words_plain']:10,.0f} | {chunk_stats.loc['max', 'tokens_plain']:12,.0f} | {chunk_stats.loc['max', 'tokens_markup']:12,.0f}")

        print(f"\n-> Wenn Maximum ({chunk_stats.loc['max', 'tokens_markup']:.0f}) größer ist als das Kontext-Fenster -> splitten.")

    # ==========================================
    # 3. GLOBALE BUCH DURCHSCHNITTE
    # ==========================================
    ...
    print("\n" + "="*85)
    print(f" {'DURCHSCHNITT & MEDIAN GANZER BÜCHER':^80} ")
    print("="*85)
