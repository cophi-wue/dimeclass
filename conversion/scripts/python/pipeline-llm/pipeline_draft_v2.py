...
import textwrap # for indentation of the LLM output
...
PROCESS_NON_CHAPTERS = False  # Wenn False, werden Front/Backmatter ignoriert
...
def wrap_tei_chunk(content: str, strategy: str) -> str:
    ...
    Indents the content and adds newlines for a clean hierarchy.
    """

    # 1. Define the indentation style (e.g., 4 spaces or \t for tab)
    indentor = "    "

    # 2. Indent the raw content from the LLM
    # This adds the indentor to the beginning of every line in 'content'
    indented_content = textwrap.indent(content, indentor)

    # 3. Determine the wrapper tag based on strategy
...
def main():
    ...
    for i, unit in enumerate(work_units):
        ...
        # 1. Spezialbehandlung für 'novel_no_separator'
        if strategy == "novel_no_separator":
            msg = "ACHTUNG novel_no_separator."
            print(f"[{i+1}/{len(work_units)}] {chunk_id}: {msg}")
            # WICHTIG: Hier müssen wir has_uncertain und has_unknown_type als False mitgeben
