...
RESULTS_FILE = os.path.join(WS, "results/hpc_glm5_eval_results.tsv")
LOG_FILE = os.path.join(WS, "logs/hpc_glm5_eval_log.txt")
...
COL_BASE = "glm5_label"
COL_SUBTYPE = "glm5_subtype"

# Context window and batch settings
MAX_MODEL_LEN = 32768  # GLM-5 context length (safe margin)
MAX_CHUNK_CHARS = 100_000  # Max chunk size to prevent token explosion
BATCH_SIZE = 200  # Batch size for continuous batching on 4× H100s
...
def main():
    ...
    logger("=" * 70)
    logger("GLM-5 Evaluation Job")
    logger(f"Model: {MODEL_ID}")
    ...
    # ─ Load tokenizer ─
    logger("")
    logger("Loading GLM-5 tokenizer...")
    try:
    ...
    logger("Initializing vLLM with tensor_parallel_size=4...")
    try:
        llm = LLM(
            model=MODEL_ID,
        ...
        logger("vLLM initialized successfully on 4× H100 GPUs.")
    except Exception as e:
