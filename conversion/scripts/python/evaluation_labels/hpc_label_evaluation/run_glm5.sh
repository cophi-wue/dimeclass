...
#SBATCH --output=logs/glm5_%j.out
#SBATCH --error=logs/glm5_%j.err
...
SCRIPT_PATH="$WS/hpc_run_evaluation_glm5.py"
if [ ! -f "$SCRIPT_PATH" ]; then
    echo "ERROR: Python script not found at $SCRIPT_PATH"
    echo "       Please copy hpc_run_evaluation_glm5.py to the workspace."
    exit 1
fi
echo "  [OK] Python script found."

# 5. Model weights cached in HF_HOME
MODEL_CACHE_DIR="$HF_HOME/hub/models--zai-org--GLM-5"
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo "  Logs     : $WS/logs/hpc_glm5_eval_log.txt"
else
    echo "  Status   : FAILED ✗"
